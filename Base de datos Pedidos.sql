...
CONSTRAINT FK_EMPLEADO_REPORTA FOREIGN KEY (REPORTA_A) REFERENCES
...
TOTAL DE VENTAS EN PESOS Y CANTIDAD X PRODUCTOS 
SELECT  P.DESCRIPCION,  SUM(DO.CANTIDAD )  , SUM(DO.CANTIDAD )  * P.PRECIOUNIT  
FROM  DETALLE_ORDENES DO 
LEFT JOIN PRODUCTOS P ON DO. PRODUCTOID = P. PRODUCTOID 
GROUP BY P.DESCRIPCION, P.PRECIOUNIT  ;
 
PROVEEDOR X CATEGORIA
SELECT NOMBREPROV, NOMBRECAT
FROM  PRODUCTOS P
INNER JOIN CATEGORIAS C ON P.CATEGORIAID = C.CATEGORIAID
INNER JOIN PROVEEDOR A ON P.PROVEEDORID = A.PROVEEDORID
GROUP BY A.NOMBREPROV, C.NOMBRECAT;
 
VENTAS X PROVEEDOR EN UNIDADES,PESOS Y STOCK
SELECT  P.DESCRIPCION,  SUM(DO.CANTIDAD )  , SUM(DO.CANTIDAD )  * P.PRECIOUNIT  
FROM  DETALLE_ORDENES DO 
LEFT JOIN PRODUCTOS P ON DO. PRODUCTOID = P. PRODUCTOID 
GROUP BY P.DESCRIPCION, P.PRECIOUNIT  ;
...
NUMERO DE ORDENES X CLIENTES
SELECT  C.NOMBRECIA , C. NOMBRECONTACTO , COUNT(*)
FROM  ORDENES O 
LEFT JOIN CLIENTES C
ON  O.CLIENTEID = C.CLIENTEID
GROUP BY C.NOMBRECIA , C. NOMBRECONTACTO ;

STOCK DE PRODUCTOS, PRECIO EN STOCK Y GANANCIAS CON IVA AL 19%
SELECT  DESCRIPCION , EXISTENCIA , ( EXISTENCIA *  PRECIOUNIT), ( EXISTENCIA *  PRECIOUNIT)*0.19 
FROM PRODUCTOS
 
PRODUCTOS QUE TIENE CADA PROVEEDOR MÁS IVA
SELECT NOMBREPROV,DESCRIPCION,EXISTENCIA ,PRECIOUNIT,PRECIOUNIT*0.19 AS PRECIO_CON_IVA
FROM PRODUCTOS P INNER JOIN PROVEEDORES PR  ON  P.PROVEEDORID = PR.PROVEEDORID 
GROUP BY PR.NOMBREPROV,  DESCRIPCION,  EXISTENCIA,  PRECIOUNIT;

PEDIDOS POR CLIENTE Y TOTAL DE DESCUENTOS
SELECT NOMBRECIA,  COUNT(*) , SUM(DESCUENTO) FROM ORDENES O 
INNER JOIN CLIENTES D ON O.CLIENTEID = D.CLIENTEID
GROUP BY  NOMBRECIA  ORDER BY COUNT(*) DESC ;

CLIENTES X EMPLEADO Y PROVEEDORES QUE USAN LOS CLIENTES
SELECT E.NOMBRE, E.APELLIDO, C. NOMBRECIA, PR.NOMBREPROV
FROM ORDENES O
LEFT JOIN EMPLEADOS E ON E.EMPLEADOID = O.EMPLEADOID
LEFT JOIN CLIENTES C  ON C.CLIENTEID = O.CLIENTEID
LEFT JOIN DETALLE_ORDENES DO ON DO.ORDENID = O.ORDENID
INNER JOIN PRODUCTOS P ON P. PRODUCTOID = DO.PRODUCTOID
INNER JOIN PROVEEDORES PR  ON P.PROVEEDORID = PR. PROVEEDORID
GROUP BY E.NOMBRE, E.APELLIDO, C. NOMBRECIA, PR.NOMBREPROV;
 
PRECIO MAYOR DE 10 Y EXISTENCIA MAYOR A 10
SELECT C.NOMBRECAT, P.DESCRIPCION, P.PRECIOUNIT, P.EXISTENCIA
FROM PRODUCTOS P
